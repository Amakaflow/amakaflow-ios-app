#!/usr/bin/env bash
#
# AMA-2457 regression gate: the AF_SESSION_IDENTITY bypass must reach the
...
#
# Usage: ./scripts/verify-auth-bypass.sh <app-path> [udid] [runs]
set -euo pipefail

APP_PATH="${1:?usage: verify-auth-bypass.sh <app-path> [udid] [runs]}"
...
for run in $(seq 1 "$RUNS"); do
  ...
  xcrun simctl launch "$UDID" "$BUNDLE_ID" \
    -AF_SESSION_IDENTITY "user_id=user_ama2457,email=baseline+clerk_test@amakaflow.dev,name=AMA2457" \
    -AF_SKIP_ONBOARDING true \
    -AF_SKIP_APPLE_WATCH true \
    -AF_USE_FIXTURES true >/dev/null 2>&1
  sleep 8
  if authed; then
    passed=$((passed + 1))
    echo "run $run: authed"
